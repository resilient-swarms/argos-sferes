...
for dim in 3 ; do   # 10 14 21 400 
    ...
    if [ $dim -gt 3 ]; then
      ...
      cmake -DLARGE=ON -DHETEROGENEOUS=ON -DNN_INPUT_TYPE=1 -DCMAKE_BUILD_TYPE=Debug -DCVT_USAGE=ON -DBD=${dim}  ..
    else
      cmake -DLARGE=ON -DHETEROGENEOUS=ON -DNN_INPUT_TYPE=1 -DCMAKE_BUILD_TYPE=Debug -DBD=${dim} ..
    fi
   make -j 2
